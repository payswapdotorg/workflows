# Workflows Architecture Lock — Version 1.0

**Status:** SUPERSEDED by Version 1.1
**Historical snapshot:** preserved unchanged from the original frozen architecture lock.

These invariants were the V1.0 frozen contract. V1.1 preserves their intent and extends them with explicit tool, resource, capability, scheduling, and session boundaries.
...
12. Hybrid is a continuous mixed-event teaching session.
...
24. A workflow may select a harness by policy but must not require provider-specific domain semantics.
...
39. Completion is established by repository merge/reconciliation authority, not by an agent claim.
